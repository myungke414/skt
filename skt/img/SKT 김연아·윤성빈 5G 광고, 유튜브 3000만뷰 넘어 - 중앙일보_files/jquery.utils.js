...
(function ($, window, document) {
	...
	*/
				...
				$list.each(function () {
					...
					if (imageSize >= 580) {
						if (images) {
						    ...
						    items.push({ Image: ($item.find('img').attr('src') || $item.find('img').data('src')), Description: $item.find('.caption').text() });
						}
					}
				});
	...
	$.fn.loadAd = function (options) {
		try {
		    ...
		    if (uagent.indexOf("googlebot") > -1 || uagent.indexOf("chrome-lighthouse") > -1) {
		        //$(this).hide();
		        return this;
		    }
			...
			var AD_INFO = {
				...
				'da_300_election2020': { src: adJs, 'data-id': '7v0WU_IXQfatEbicQrgCXQ', name: 'joongang_p/article/article@415_election_300x250?mlink=454', adtype: "js", displaytype: "single", isshapepopup: false }, //2020총선
				...
				'home_ci_806': { src: adJs, 'data-id': 'KSgeBN4LTwytMIb5CVgjZg', name: 'joongang_p/main/main@main_house_290x60?mlink=429"', adtype: "js", displaytype: "single", isshapepopup: false }, //홈 제호옆 베너
				...
				'favorite_ad': { width: '260', height: '36', src: 'https://media.adpnut.com/cgi-bin/PelicanC.dll?impr?pageid=03OA&out=iframe', title: '많이본 기사 하단 AD', id: 'favorite_ad', name: 'favorite_ad' },  //많이본 기사 하단 AD
				'mainTopAd': { src: adJs, 'data-id': 'baW-8g5OQgqA7A-s65B1Hg', name: 'joongang_p/main/main@main_top_1060x317?mlink=367', adtype: "js", displaytype: "single", isshapepopup: false, closeButton: true },// mainTopAd
				'mainMiddleAd': { src: adJs, 'data-id': 'dkBYPKdeS-WWgSmqMBJLRQ', name: 'joongang_p/main/main@main_middle_1060x317?mlink=368', adtype: "js", displaytype: "single", isshapepopup: false, closeButton: true },// mainMiddleAd
				'mainBottomAd': { src: adJs, 'data-id': '_PpgS5orSIadENAjLnS7pQ', name: 'joongang_p/main/main@main_bottom_1060x317?mlink=369', adtype: "js", displaytype: "single", isshapepopup: false, closeButton: true },// mainBottomAd
				'joongang_pc_200x200_1_test': { width: '200', height: '200', src: '//adex.ednplus.com/xc/h/tbspW8tb' }, //아티클 하단 쇼핑박스1
				'joongang_pc_200x200_2_test': { width: '200', height: '200', src: '//adex.ednplus.com/xc/h/tgFPQ4CJ' }, //아티클 하단 쇼핑박스2
				'joongang_pc_200x200_3_test': { width: '200', height: '200', src: '//adex.ednplus.com/xc/h/tgKddDqq' }, //아티클 하단 쇼핑박스3
				...
				'left_915_no_money': { src: adJs, 'data-id': 'KyC3K5LcTMWHUJgVMzRScg', name: 'joongang_p/article/article@article_top_left_network_120x600?mlink=462', adtype: "js", displaytype: "single", isshapepopup: false },  //경제 제외한 윙배너
				...
				'da_article_right_middle': { src: adJs, 'data-id': 'HI4-9wv6QG2jGIO4FhcdpQ', name: 'joongang_p/article/article@article_right_middle_banner?mlink=405', adtype: "js", displaytype: "single", isshapepopup: false } // 아티클 우측 중간 배너
			};
			var $p = this,
			...
			if (attr.adtype === "js") {
				switch (attr.displaytype) {
					case "multi":
						...
						break;
				    case "single":
				        if (utils.config('pageType') == PAGE_TYPE.pcHome) {
				            $p.append('<div class="inner"><div class="bd"><div id="imp_ad_' + config.type + '"></div>' + '</div>' + (attr.closeButton == true ? '<a class="btn_close"></a>' : '') + '</div>');
				        }
				        else {
				...
				if (typeof AD_INFO[config.type]['data-id'] != 'undefined')
				    script.setAttribute('data-id', AD_INFO[config.type]['data-id']);
				if (typeof AD_INFO[config.type]['name'] != 'undefined')
	...
	*/
	$.parallaxScrolling = function () {
	    if (utils.config('pageType') != PAGE_TYPE.article)
	        return false;
		...
		function initWidePosition() {
			...
			if ($t.length > 0 && $t.is(':visible')) {
				if ($t.css('position') == 'fixed') {
					var newPosiLeft = ($wrap.width() + $wrap.offset().left);
		...
		function stopPosition(targetObj) {
			...
			if (targetObj.type == "sub") {
				if (menuKey != 'find') {
					...
					styles.bottom = ($('.footer').height() > targetObj.bottom) ? parseFloat(targetObj.$ele.css('padding-bottom').replace("px", ""), 10) : targetObj.bottom - $('.footer').height() - parseFloat(targetObj.$ele.css('margin-bottom').replace("px", ""), 10);
				}
			}
			...
			if (targetObj.type == "wide") {
				styles.bottom = 0;
				nPosiLeft = ($wrap.width() + $wrap.offset().left) - parseFloat($(window).scrollLeft(), 10);
				targetObj.$ele.css({ 'left': parseFloat(nPosiLeft, 10) });
	...
	*/
	$.fn.searchAutoComplete = function (options) {
						...
						if (res.responsestatus !== -1) { // -1: 실패 , 0 : 성공
							...
							if (result.totalcount > 0) {
								...
								$.each(items, function (i, v) {
									...
									if (location.href.indexOf('joongang.joins.com') > -1)
									    url = utils.config('searchWebPath') + '?keyword=' + encodeURI(text);
		...
		function renderFoot($wrap) {
			...
			if (config.enabledRemoveHistory && config.enabledStopSaving && location.href.indexOf('joongang.joins.com') == 0) {
				foot += ' | ';
			}
