    ...
    $.fn.comment = function (options) {
        ...
        (function () {
            $.when.apply($,
                ...
                //선거
                //$("#comment").addClass("election_comment");
                init();
        ...
        function userLogin(type, device) {
            ...
            if (type === 'joins' || type === 'kakao' || type === 'facebook' || type === 'twitter' || type === 'apple') {
        ...
        function renderForm($el) {
                ...
                'select_sns': {
                    list: {
                        item: {
                            html: function () {
                                ...
                                if (this.code === 'joins' || this.code === 'kakao' || this.code === 'apple') {
                    ...
                    'sns_cmt_fb': {
                        html: function () {
                            ...
                            if (userInfo.type !== 'joins' && userInfo.type !== 'kakao' && userInfo.type !== 'twitter' && userInfo.type !== 'facebook' && userInfo.type !== 'apple') {
            ...
            if ($form.find('.select_sns').length === 0) {
                ...
                var regLink = "https://my.joins.com/register/regist.asp?TargetURL=" + href;
                $("#joins_register").attr('href', regLink);
                if (isLogin === true && userInfo.type !== "joins") {
                    var decHtml = '<span class="dsc">선거운동 기간 중에는 공직선거법 제82조의6 제1항에 따라 정당∙후보자에 대한 지지∙반대의 정보 게시할 경우 실명확인이 필요합니다.</span>';
                    $(".content").attr('readonly', 'readonly');
                    $(".content").after(decHtml);
                }*/
            }
        ...
        return this.each(function () {
            ...
            $container.on('click', '.btn_logout', function () {
                ...
                //}).on('click', '#layer_social_display', function () {//선거댓글
                $(this).next().toggle();
                ...
                //if (userInfo.type === "joins" && isLogin) {
                    $(this).closest('.cmt_area').next().toggle();
                //} else {
                //    $(".layer_social_comment").show();
